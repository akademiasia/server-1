...
class UpdateConfig extends Command {
	...
	protected function execute(InputInterface $input, OutputInterface $output): int {
		...
		if ($key === null) {
			$output->writeln('<info>Current theming config:</info>');
			foreach (self::SUPPORTED_KEYS as $key) {
				$value = \OC::$server->getConfig()->getAppValue('theming', $key, null);
				if ($value) {
		...
		if (in_array($key, ['background', 'logo', 'favicon', 'logoheader'], true)) {
		...
		$output->writeln('<info>Updated config</info>');
